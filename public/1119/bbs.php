...
$select_sth = $dbh->prepare(
  'SELECT bbs_entries.*,' .
  '  (SELECT name FROM users WHERE users.id = bbs_entries.user_id LIMIT 1) AS user_name,' .
  '  (SELECT icon_filename FROM users WHERE users.id = bbs_entries.user_id LIMIT 1) AS icon_filename' .
  ' FROM bbs_entries' .
    ...
    <dd>
      ...
      <?php else: ?>
        <span style="display:inline-block;width:40px;height:40px;border-radius:50%;
                     background:#ddd;text-align:center;line-height:40px;margin-right:8px;">
          ?
        </span>
      <?php endif; ?>
